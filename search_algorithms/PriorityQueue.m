...
%
%  Modified by Matthew Giamou January 23, 2019
...
classdef PriorityQueue < handle
   ...
   methods
       ...
       function obj = PriorityQueue(N)
           
           obj.userData{N} = 0;
           obj.priority = zeros(N,1);
           obj.data = zeros(N,1);
           obj.state_lookup = containers.Map('KeyType', 'int32', 'ValueType', 'int32');
       end
       ...
       function [element_data,element_priority] = pop(obj)
           ...
           % Suppress warnings for "dead pops" - i.e. a better 
           % path to this node has already been explored
           if isKey(obj.state_lookup, element_data.state)
               obj.state_lookup.remove(element_data.state);
           end
       ...
       function push(obj, element_priority, element_data)
            ...
            obj.state_lookup(element_data.state) = obj.inputIndex;
       ...
       function [element_data] = get_by_state(obj, state)
           element_data = obj.getUserData(obj.state_lookup(state));
       end
       
       function [is_present] = contains_state(obj, state)
           is_present = isKey(obj.state_lookup, state);
       end
